...
async def report_cd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    DATA['reports'][rep_id] = {
        ...
        'status': 'pending', 'at': datetime.now()
    }
    DATA['report_counter'] += 1

    # Уведомление владельцам и админам
    txt = f"📋 <b>ОТЧЕТ #{rep_id}</b>\n👤 Ник: {context.user_data['nick']}\n💰 {format_balance(context.user_data['payment'])} ₽\n💳 Счёт: {user['bank_account']}"
    btn = [[InlineKeyboardButton("✅ ОДОБРИТЬ", callback_data=f"appr_{rep_id}")]]

    for o_id in get_all_privileged():
        try:
            await context.bot.send_photo(o_id, context.user_data['photo_start'], caption="📸 Фото <b>НАЧАЛА</b> стройки", parse_mode=ParseMode.HTML)
            await context.bot.send_photo(o_id, context.user_data['photo_end'], caption=txt, reply_markup=InlineKeyboardMarkup(btn), parse_mode=ParseMode.HTML)
        except Exception as e:
...
async def approve_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rep['status'] == 'pending':
        ...
        for o_id in get_all_privileged():
            if o_id != query.from_user.id:
                try:
                    await context.bot.send_message(o_id, f"ℹ️ Отчет #{rep_id} ({rep['nick']}) уже одобрен пользователем {approver_name}.")
                except Exception: pass
