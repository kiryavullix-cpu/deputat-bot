...
С сохранением данных в файл (data.json)
"""
...
def calculate_cd_expiry(cd_minute):
    """Вычисляет ближайшее время когда наступит cd_minute."""
    now = datetime.now()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id) and user_id not in DATA['users']:
        DATA['users'][user_id] = {
            ...
            'has_access': True, 'balance': 0, 'total_reports': 0, 'bank_account': None, 'nick': None, 'blocked': False
        }
        save_data()
...
async def report_cd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Устанавливаем КД сразу при отправке отчёта
    ...
    save_data()
    await update.message.reply_text(f"✅ Отчет #{rep_id} отправлен! КД до: {cd_min} минуты")
    context.user_data.clear()
...
async def approve_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if rep['status'] == 'pending':
        ...
        approved_txt = (f"📋 <b>ОТЧЕТ #{rep_id}</b>\n"
                        ...
                        f"⏰ КД до: {rep['cd']} минуты\n\n"
                        f"✅ <b>ОДОБРЕНО</b> — {approver_name}")

        # Редактируем сообщение с кнопкой у всех — убираем кнопку и ставим ОДОБРЕНО
        ...
        # Если старый формат (нет msg_ids) — редактируем текущее
...
async def show_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in users:
        ...
        text = (f"{role} <b>{u['full_name']}</b>{nick}\n"
                f"ID: <code>{u['id']}</code>\n"
                f"Статус: {status}\n"
                f"💰 Баланс: {format_balance(u['balance'])} ₽")
        btns = []
...
async def cmd_givebonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_data()
    await update.message.reply_text(
        f"✅ Выдан бонус <b>{format_balance(amount)} ₽</b> пользователю {user['full_name']}.\n"
...
async def cmd_takecash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_data()
    await update.message.reply_text(
        f"✅ Снято <b>{format_balance(amount)} ₽</b> у пользователя {user['full_name']}.\n"
        f"Причина: {reason}\nНовый баланс: {format_balance(user['balance'])} ₽", parse_mode=ParseMode.HTML)
    try:
...
async def cmd_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    await update.message.reply_text(f"🆔 Ваш Telegram ID: <code>{uid}</code>", parse_mode=ParseMode.HTML)
...
async def cmd_otcets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not reps:
        await update.message.reply_text(f"📋 У пользователя {user['full_name']} нет отчётов.")
        return
    header = f"📋 <b>Отчёты пользователя {user['full_name']}</b> (всего: {len(reps)})\n\n"
