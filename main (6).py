...
async def cmd_egive(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not DATA["event"].get("active"):
        await update.message.reply_text("⚠️ Нет активного конкурса."); return
    if not ctx.args:
        await update.message.reply_text("❌ /egive [ID]"); return
...
async def cmd_givebonus(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    args = ctx.args
    if len(args) < 3:
        await update.message.reply_text("❌ /givebonus [ID] [сумма] [причина]"); return
    try: tid = int(args[0]); amount = int(args[1]); reason = " ".join(args[2:])
    except ValueError:
        await update.message.reply_text("❌ ID и сумма — числа."); return
    user = DATA["users"].get(tid)
    if not user:
...
async def cmd_takecash(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    args = ctx.args
    if len(args) < 3:
        await update.message.reply_text("❌ /takecash [ID] [сумма] [причина]"); return
    try: tid = int(args[0]); amount = int(args[1]); reason = " ".join(args[2:])
    except ValueError:
        await update.message.reply_text("❌ ID и сумма — числа."); return
    user = DATA["users"].get(tid)
...
async def cmd_makeadmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id): return
    if not ctx.args:
        await update.message.reply_text("❌ /makeadmin [ID]"); return
    try: tid = int(ctx.args[0])
    except ValueError:
        await update.message.reply_text("❌ ID — число."); return
    if is_owner(tid):
...
async def cmd_takeadmin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id): return
    if not ctx.args:
        await update.message.reply_text("❌ /takeadmin [ID]"); return
    try: tid = int(ctx.args[0])
    except ValueError:
        await update.message.reply_text("❌ ID — число."); return
...
async def cmd_userinfo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Только для администраторов."); return
    if not ctx.args:
        await update.message.reply_text("❌ /userinfo [ID]"); return
    try: tid = int(ctx.args[0])
    except ValueError:
        await update.message.reply_text("❌ ID — число."); return
    user = DATA["users"].get(tid)
...
async def cmd_otcets(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Только для администраторов."); return
    if not ctx.args:
        await update.message.reply_text("❌ /otcets [ID]"); return
    try: tid = int(ctx.args[0])
    except ValueError:
        await update.message.reply_text("❌ ID — число."); return
    user = DATA["users"].get(tid)
